...
def save_to_sheet(port, 장비명, 고장구분, 중분류, teams, 상세내역, 알람=''):
    try:
        sheet = get_sheet()
        ...
        for i, r in enumerate(records):
            if r.get('Port') == port:
                sheet.update(f'A{i+2}:G{i+2}',
                    [[port, 장비명, 고장구분, 중분류, teams, 상세내역, 알람]])
                return True
        sheet.append_row([port, 장비명, 고장구분, 중분류, teams, 상세내역, 알람])
        return True
    except Exception:
        return False
...
with tab1:
    ...
    disp = df_batch[[
        ...
        '고장구분','고장구분(중분류)','다발','알람','복구/미복구 상세내역'
    ]].copy()
    ...
    if selected:
        ...
        with col_a:
            ...
            unit_type = st.text_input("Unit Type (AN)",
                ...
                key=f"ut_{sel_port}")
            alarm = st.text_input("알람 (AQ)",
                value=saved.get('알람') or (str(df_batch.loc[sel_idx,'알람']) if pd.notna(df_batch.loc[sel_idx,'알람']) and df_batch.loc[sel_idx,'알람'] != '' else ""),
                key=f"al_{sel_port}")
        with col_b:
        ...
        if st.button("💾 저장 (Google Sheets)", key="save_batch", use_container_width=True):
            ok = save_to_sheet(sel_port, sel_name, gj, gj_mid, teams, detail, alarm)
            if ok:
