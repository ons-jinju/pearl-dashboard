...
""", unsafe_allow_html=True)
        # 다운로드 링크 (HTML a태그 - CSS 충돌 없음)
        import base64
        _csv_bytes = df_today.to_csv(index=False).encode('utf-8-sig')
        _b64 = base64.b64encode(_csv_bytes).decode()
        st.markdown(
            f'<a href="data:text/csv;charset=utf-8-sig;base64,{_b64}" '
            f'download="금일알람_{today_str}.csv" '
            f'style="display:block;text-align:center;padding:10px;margin:8px 0;'
            f'background:#1971c2;color:#fff;border-radius:8px;font-weight:600;'
            f'text-decoration:none;font-size:.9rem;">⬇️ 금일 알람 다운로드 ({today_str})</a>',
            unsafe_allow_html=True
        )
        st.markdown("---")
...
with tab7:
    if df_vswr.empty:
        ...
        fig_v = px.histogram(
            dfv.dropna(subset=['VSWR_num']),
            x='VSWR_num', nbins=20,
            color_discrete_sequence=['#1971c2']
        )
        fig_v.update_layout(xaxis_title='VSWR', yaxis_title='건수')
