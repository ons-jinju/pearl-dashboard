...
def load_all(filepath):
    ...
    for s in ['중계기 및 MIBOS 알람', 'gREMS',
        ...
        if s in sheets:
            data[s] = pd.read_excel(filepath, sheet_name=s)
...
with tab1:
    ...
    st.caption("국소를 선택하고 작업 내용을 입력하세요. 입력 후 엑셀 다운로드 버튼으로 저장하세요.")
    ...
    if selected:
        ...
        # 기존 저장값 불러오기
        saved = st.session_state.batch_edits.get(sel_port, {})

        col_a, col_b = st.columns(2)
        with col_a:
            gj = st.text_input("고장구분 (AL)",
                value=saved.get('고장구분', str(df_batch.loc[sel_idx,'고장구분']) if pd.notna(df_batch.loc[sel_idx,'고장구분']) else ""),
                key=f"gj_{sel_port}")
            gj_mid = st.text_input("고장구분(중분류) (AM)",
                value=saved.get('고장구분(중분류)', str(df_batch.loc[sel_idx,'고장구분(중분류)']) if pd.notna(df_batch.loc[sel_idx,'고장구분(중분류)']) else ""),
                key=f"gjm_{sel_port}")
            unit_type = st.text_input("Unit Type (AN)",
                value=saved.get('Unit Type', str(df_batch.loc[sel_idx,'Unit Type']) if pd.notna(df_batch.loc[sel_idx,'Unit Type']) else ""),
                key=f"ut_{sel_port}")
        with col_b:
            teams = st.text_input("Teams 등록여부 (AO)",
                value=saved.get('Teams 등록여부', str(df_batch.loc[sel_idx,'Teams 등록여부']) if pd.notna(df_batch.loc[sel_idx,'Teams 등록여부']) else ""),
                key=f"tm_{sel_port}")
            detail = st.text_area("복구/미복구 상세내역 (AP)",
                value=saved.get('복구/미복구 상세내역', str(df_batch.loc[sel_idx,'복구/미복구 상세내역']) if pd.notna(df_batch.loc[sel_idx,'복구/미복구 상세내역']) else ""),
                height=120,
                key=f"dt_{sel_port}")

        if st.button("💾 입력 저장", key="save_batch"):
            st.session_state.batch_edits[sel_port] = {
                '고장구분': gj,
                '고장구분(중분류)': gj_mid,
                'Unit Type': unit_type,
                'Teams 등록여부': teams,
    ...
    if st.session_state.batch_edits:
        ...
        summary_rows = []
        for p, v in st.session_state.batch_edits.items():
            summary_rows.append({
                'Port': p,
                '고장구분': v.get('고장구분',''),
                '중분류': v.get('고장구분(중분류)',''),
    ...
    # ── 엑셀 다운로드 ────────────────────────────────────────────
    st.markdown('<div style="font-size:14px;font-weight:700;color:#1a5c8a;margin:16px 0 8px 0;">📥 엑셀 다운로드</div>', unsafe_allow_html=True)

    import io
    def make_excel():
        df_out = df.copy()
        # 수기 입력값 반영
        for port, vals in st.session_state.batch_edits.items():
            mask = df_out['Port'] == port
            for col, val in vals.items():
                if col in df_out.columns and val:
                    df_out.loc[mask, col] = val

        buf = io.BytesIO()
        with pd.ExcelWriter(buf, engine='openpyxl') as writer:
            df_out.to_excel(writer, sheet_name='5G_LTE OOS_진주', index=False)
            # 다른 시트도 함께
            for s in ['중계기 및 MIBOS 알람','gREMS',
                      'RMS_A망 미복구','RMS_DACS 미복구','RMS_통합RCU미복구',
                      'NO-CALL현황','MIBOS AMP 미사용']:
    ...
    st.download_button(
        label="📥 수정된 엑셀 다운로드",
        data=make_excel(),
        file_name=f"26년_진주품질개선팀_고장_RAW_DATA_{latest_date}.xlsx",
...
with tab4:
    ...
    if len(df_rep) > 0:
        ...
        if 'rep_edits' not in st.session_state:
            st.session_state.rep_edits = {}
        ...
        if sel_rep:
            ...
            rep_saved = st.session_state.rep_edits.get(sel_name, "")
            # 기존 작업내용 불러오기
            existing = df_rep[df_rep['중계기명'] == sel_name]['작업내용'].values
            default_val = rep_saved if rep_saved else (str(existing[0]) if len(existing) > 0 and pd.notna(existing[0]) else "")
            ...
            if st.button("💾 작업내용 저장", key="save_rep"):
                st.session_state.rep_edits[sel_name] = rep_detail
                st.success(f"✅ {sel_name} 저장 완료!")

        # 저장된 항목 요약
        if st.session_state.rep_edits:
            ...
            rep_summary = [{'중계기명': k, '작업내용': v} for k, v in st.session_state.rep_edits.items()]
            st.dataframe(pd.DataFrame(rep_summary), use_container_width=True, height=150)

        # 중계기 엑셀 다운로드
        import io as _io
        def make_rep_excel():
            df_rep_out = df_rep.copy()
            for name, val in st.session_state.rep_edits.items():
                mask = df_rep_out['중계기명'] == name
                df_rep_out.loc[mask, '작업내용'] = val
            buf = _io.BytesIO()
            with pd.ExcelWriter(buf, engine='openpyxl') as writer:
                df_rep_out.to_excel(writer, sheet_name='중계기 및 MIBOS 알람', index=False)
            buf.seek(0)
            return buf

        st.download_button(
            label="📥 중계기 작업내용 엑셀 다운로드",
            data=make_rep_excel(),
            file_name=f"중계기_MIBOS_알람_{latest_date}.xlsx",
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            use_container_width=True,
        )
    else:
