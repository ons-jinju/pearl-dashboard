...
def load_all(filepath):
    ...
    # 13시 알람 공유 시트
    if '13시 알람 공유' in sheets:
        d13 = pd.read_excel(filepath, sheet_name='13시 알람 공유')
        if '복구내용' not in d13.columns:
            d13['복구내용'] = ''
        data['13시 알람 공유'] = d13

    # 종합 시트
    if '종합' in sheets:
        data['종합'] = pd.read_excel(filepath, sheet_name='종합', header=None)
...
with tab1:
    ...
    # ── 카드형 표시 ───────────────────────────────────────────────
    badge_style = {
        '미복구': ('background:#ffe8e8;color:#c0392b;', '미복구'),
        '점검중': ('background:#fff0d6;color:#b45309;', '점검중'),
        '신규':   ('background:#dbeeff;color:#1a5c8a;', '신규'),
        '복구':   ('background:#d6f5e8;color:#1a7a50;', '복구'),
    }
    border_color = {
        '미복구': '#e05c5c',
        '점검중': '#f5a623',
    ...
    for _, row in df_batch.iterrows():
        ...
        시스템  = row.get('시스템', '')
        시군구  = row.get('시군구', '')
        고장구분= str(row.get('고장구분', '')).replace('nan','')
        중분류  = str(row.get('고장구분(중분류)', '')).replace('nan','')
        다발    = row.get('다발', '-')
        알람    = row.get('알람', '')
        상세    = str(row.get('복구/미복구 상세내역', '')).replace('nan','')
        발생일시= str(row.get('발생시각', ''))

        bg, label = badge_style.get(구분, ('background:#eee;color:#666;', 구분))
        border     = border_color.get(구분, '#ccc')

        알람_html  = f'<div style="font-size:11px;color:#3a5c9a;background:#f0f4ff;padding:3px 8px;border-radius:6px;margin-top:4px;display:inline-block;">🔔 {알람}</div>' if 알람 else ''
        다발_html  = f'<span style="background:#fff0d6;color:#b45309;font-size:10px;padding:2px 6px;border-radius:6px;margin-left:6px;">{다발}</span>' if 다발 != '-' else ''
        상세_html  = f'<div style="font-size:11px;color:#555;margin-top:5px;padding:4px 8px;background:#f8f8f8;border-radius:6px;">💬 {상세}</div>' if 상세 else ''
        고장_txt   = f'&nbsp;|&nbsp; {고장구분}' if 고장구분 else ''
        중분류_txt = f'&nbsp;|&nbsp; {중분류}' if 중분류 else ''

        cards_html += f"""
        <div style="background:#fff;border-radius:12px;padding:12px 14px;margin-bottom:8px;
                    border-left:4px solid {border};box-shadow:0 1px 6px rgba(0,0,0,0.07);">
          <div style="display:flex;align-items:center;gap:8px;margin-bottom:4px;">
            <span style="{bg}font-size:11px;font-weight:700;border-radius:10px;padding:2px 9px;">{label}</span>
            <span style="font-size:13px;font-weight:700;color:#1a3a5c;">{장비명}</span>
            {다발_html}
          </div>
          <div style="font-size:11px;color:#4a7a9b;font-family:'JetBrains Mono',monospace;">{port}</div>
          <div style="font-size:11px;color:#666;margin-top:4px;">
            🕐 {발생일시} &nbsp;|&nbsp; {시스템} &nbsp;|&nbsp; {시군구}{고장_txt}{중분류_txt}
          </div>
          {알람_html}
          {상세_html}
        </div>
...
# ══ TAB2 13시 알람 ═════════════════════════════════════════════════
with tab2:
    st.markdown('<h4 style="color:#1a5c8a;margin-bottom:12px;">🕐 13시 알람 공유</h4>', unsafe_allow_html=True)

    df_13 = data.get('13시 알람 공유')
    if df_13 is not None and len(df_13) > 0:
        # Google Sheets에서 13시 알람 복구내용 불러오기
        _, _ = load_from_sheet()

        st.caption(f"총 {len(df_13)}건")
        st.dataframe(df_13, use_container_width=True, height=300)

        # 복구내용 수기 입력
        st.markdown('<div style="font-size:14px;font-weight:700;color:#1a5c8a;margin:16px 0 8px 0;">✏️ 복구내용 입력</div>', unsafe_allow_html=True)

        # 장비명 선택
        name_opts = df_13['장비명'].tolist()
        sel_13 = st.selectbox("📍 장비 선택", name_opts, key="sel_13")

        if sel_13:
            sel_13_idx = df_13[df_13['장비명'] == sel_13].index[0]
            # 기존 복구내용 불러오기
            sheet_13, _ = load_from_sheet()
            saved_13 = sheet_13.get(f'[13시]{sel_13}', {})
            existing_13 = df_13.loc[sel_13_idx, '복구내용'] if '복구내용' in df_13.columns else ''
            default_13 = saved_13.get('복구/미복구 상세내역') or (str(existing_13) if pd.notna(existing_13) and existing_13 != '' else '')

            c13a, c13b = st.columns(2)
            with c13a:
                gj_13 = st.text_input("고장구분",
                    value=saved_13.get('고장구분', ''),
                    key="gj_13")
            with c13b:
                gj_mid_13 = st.text_input("고장구분(중분류)",
                    value=saved_13.get('고장구분(중분류)', ''),
                    key="gjm_13")

            detail_13 = st.text_area("복구내용",
                value=default_13,
                height=100,
                key="dt_13")

            if st.button("💾 복구내용 저장 (Google Sheets)", key="save_13", use_container_width=True):
                ok = save_to_sheet(
                    f'[13시]{sel_13}', sel_13,
                    gj_13, gj_mid_13, '', detail_13
                )
                if ok:
                    st.success(f"✅ {sel_13} Google Sheets 저장 완료!")
                    st.cache_resource.clear()
                else:
                    st.error("❌ 저장 실패. 잠시 후 다시 시도해주세요.")

        # 저장된 목록 표시
        sheet_13_all, _ = load_from_sheet()
        sheet_13_filtered = {k.replace('[13시]',''):v for k,v in sheet_13_all.items() if k.startswith('[13시]')}
        if sheet_13_filtered:
            st.markdown('<div style="font-size:13px;font-weight:700;color:#1a5c8a;margin:12px 0 6px 0;">📋 입력 완료 목록</div>', unsafe_allow_html=True)
            rows_13 = [{'장비명': k, '고장구분': v.get('고장구분',''), '복구내용': v.get('복구/미복구 상세내역','')} for k,v in sheet_13_filtered.items()]
            st.dataframe(pd.DataFrame(rows_13), use_container_width=True, height=200)
    else:
        st.markdown('<div class="empty-bar">📭 현재 13시 알람 데이터가 없습니다</div>', unsafe_allow_html=True)
...
with tab3:
    ...
    # 알람 컬럼 없으면 추가, 있으면 nan 처리
    if '알람' not in df_raw.columns:
        df_raw['알람'] = ''
    else:
        df_raw['알람'] = df_raw['알람'].fillna('').astype(str).replace('nan', '')

    disp_raw = df_raw[['발생시각','장비명','Port','시스템','시군구',
                        '고장구분','고장구분(중분류)','다발','알람','복구/미복구 상세내역']].copy()
    disp_raw.columns = ['발생일시','장비명','Port','시스템','시군구',
                        '고장구분','중분류','다발','알람','복구사유/상세내역']
