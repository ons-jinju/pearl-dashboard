...
# ── 파일 경로 (GitHub 저장소 내 고정 파일) ────────────────────────
FILE_NAME = "26년_진주품질개선팀_고장_RAW_DATA.xlsx"
FILE_PATH = os.path.join(os.path.dirname(__file__), FILE_NAME)

if not os.path.exists(FILE_PATH):
    st.error(f"⚠️ 파일을 찾을 수 없습니다: `{FILE_NAME}`")
    st.info("GitHub 저장소에 엑셀 파일이 업로드되어 있는지 확인해주세요.")
    st.stop()

st.markdown(f"""
<div class="info-bar">📂 데이터 파일: <b>{FILE_NAME}</b> &nbsp;|&nbsp; 마지막 수정: <b>{pd.Timestamp(os.path.getmtime(FILE_PATH), unit='s').strftime('%Y-%m-%d %H:%M') if os.path.exists(FILE_PATH) else '-'}</b></div>
""", unsafe_allow_html=True)
...
def load_all(filepath):
    ...
    if '중계기 및 MIBOS 알람' in sheets:
        data['rep'] = pd.read_excel(filepath, sheet_name='중계기 및 MIBOS 알람')

    # gREMS
    if 'gREMS' in sheets:
        data['grems'] = pd.read_excel(filepath, sheet_name='gREMS')
...
data, sheet_names = load_all(FILE_PATH)
df = data['main']
...
def calc_batch(df):
    ...
    # ① 최신 배치일 = A열 기준 최신일 (12월31일 이상값 제외)
    ...
    # ③ 복구 여부 판단
    #    - 고장구분 = 복구 or 상세내역에 복구 포함 → 복구
    #    - 그 외 (Unit/BP/출입불가 등) → 미복구
    #    - 최신 배치에 없는 Port → 복구 (자동복구)
    def judge_status(row):
        gj     = str(row.get('고장구분', '')).strip()
        detail = str(row.get('복구/미복구 상세내역', '')).strip()
        if pd.isna(row.get('고장구분')):
            return '미복구'
    ...
    # ④ 다발 계산: 동일 Port, 발생시각 다른 것 (전체 데이터 기준)
...
# ── 대시보드 통계 ─────────────────────────────────────────────────
def get_stats(df):
    # 전체 실건수: Port + 발생시각 중복제거
    df_dedup = df.drop_duplicates(subset=['Port', '발생시각'])
    total    = len(df_dedup)
    unfix    = len(df_batch[df_batch['상태'] == '미복구'])
    fix      = total - unfix  # 실고장 - 미복구 = 복구
    # 다발 국소: 동일 Port, 발생시각 다른 것 2건 이상
    dabal_total = sum(1 for v in dabal_map.values() if v > 1)
    g5  = len(df_dedup[df_dedup['시스템'] == '5G'])
    lte = len(df_dedup[df_dedup['시스템'] == 'LTE'])

    def count_cat(d, cat):
        return len(d[d['고장구분'].str.strip().str.upper() == cat.upper()])

    cats_labels = ['복구', 'Unit', 'BP', '재발생', '점검중', '기타']
    def get_cats(d):
        fix_n    = len(d[d['고장구분'].str.strip().str.lower() == '복구'])
        unit_n   = len(d[d['고장구분'].str.strip().str.upper() == 'UNIT'])
        bp_n     = len(d[d['고장구분'].str.strip().str.upper() == 'BP'])
        re_n     = len(d[d['고장구분'].str.strip() == '재발생'])
        check_n  = len(d[d['고장구분'].str.strip() == '점검중'])
        etc_n    = len(d[~d['고장구분'].str.strip().str.lower().isin(['복구','unit','bp','재발생','점검중',''])])
        return [fix_n, unit_n, bp_n, re_n, check_n, etc_n]
...
with tab1:
    ...
    with c2:
        fig2 = go.Figure(go.Bar(x=stats['cats_labels'], y=stats['cats_vals'], marker_color=colors_bar))
...
with tab4:
    ...
    if 'rep' in data and data['rep'] is not None and len(data['rep']) > 0:
        df_rep = data['rep']
        ...
        for col, (v, lbl, clr) in zip(c_rep, [
            ...
            (str(len(df_rep)),"CDMA+WCDMA",    "#f5a623"),
        ]):
...
with tab5:
    ...
    if 'grems' in data and data['grems'] is not None and len(data['grems']) > 0:
        df_gr = data['grems']
